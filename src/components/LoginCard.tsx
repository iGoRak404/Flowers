...
export const LoginCard: React.FC<LoginCardProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Verificación de credenciales con transición visual suave (Paso 1 del timeline)
    setTimeout(() => {
      ...
      if (foundUser) {
        playGentleSparkle();
        setIsExiting(true); // Desvanecimiento suave del formulario
        setTimeout(() => {
  ...
  return (
    <div
      ...
      className={`relative z-20 w-full max-w-md mx-auto px-4 py-8 sm:px-6 transition-all duration-500 ease-out ${
    ...
    >
      ...
      <div className="glass-panel golden-card-glow rounded-3xl p-6 sm:p-8 shadow-2xl relative overflow-hidden transition-all duration-300">
        ...
        <div className="text-center mb-7">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-tr from-amber-500 to-yellow-300 text-slate-950 shadow-lg shadow-amber-500/30 mb-3 transform hover:scale-105 transition-transform duration-300">
          ...
          <p className="text-sm text-amber-200/80 mt-1.5 font-light">
            Inicia sesión
          </p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-900/60 border border-amber-500/25 text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all shadow-inner"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-11 py-3 rounded-xl bg-slate-900/60 border border-amber-500/25 text-white placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all shadow-inner"
              />
              <button
                type="button"
                id="toggle-password-visibility-btn"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-amber-300/60 hover:text-amber-300 focus:outline-none"
